...
for n=1:Nsteps % begin time stepping
    ...
    u = [k1(M),k1(1:M-1)]; % k1_{m-1}
    f1  = (up1-u)/dx;  % f(k1) using central_2
    k2  = u + 0.5*dt*f1;     % compute k2 using dt/2
   
    up1 = [k2(2:M),k2(1)];   % k2_{m+1}
    u = [k2(M),k2(1:M-1)]; % k2_{m-1}
    f2  = (up1-u)/dx;  % f(k2) using central_2
    k3  = u + 0.5*dt*f2;     % compute k3 using dt/2
    
    up1 = [k3(2:M),k3(1)];   % k3_{m+1}
    u = [k3(M),k3(1:M-1)]; % k3_{m-1}
    f3  = (up1-u)/dx;  % f(k3) using central_2
    k4  = u + dt*f3;         % compute k4 using dt 
    
    up1 = [k4(2:M),k4(1)];   % k4_{m+1}
    u = [k4(M),k4(1:M-1)]; % k4_{m-1}
    f4  = (up1-u)/dx;  % f(k4) using central_2
